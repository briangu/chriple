...
module Chasm {
  ...
  record InstructionReader {
    ...
    inline proc readTriple(): Triple {
      return
        ((instructions.read(): Triple) << 24) |
        ((instructions.read(): Triple) << 16) |
        ((instructions.read(): Triple) << 8) |
        (instructions.read(): Triple);
    }
  }
  ...
  record InstructionWriter {
    ...
    proc write_triple(triple: Triple): bool {
      if (!instructions.canAdvance(4)) {
        error("write_push is out of instruction space for term: ", triple, " at offset ", instructions.offset);
        return false;
      }

      instructions.write((triple >> 24): ChasmOp);
      instructions.write(((triple & 0x00FF0000) >> 16): ChasmOp);
      instructions.write(((triple & 0x0000FF00) >> 8): ChasmOp);
      instructions.write(triple: ChasmOp);
    ...
    proc write_push_term(triple: Triple): bool {
